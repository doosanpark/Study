...
객체란 서로 연관된 변수와 함수를 **그룹핑한 그릇**이라고 할 수 있다. 
...
함수를 호출할 때 new을 붙이면 새로운 객체를 만든 후에 이를 리턴한다. 위의 코드는 새로운 객체를 변수 p에 담았다. 여러사람을 위한 객체를 만들 경우 코드가 계속 길어질 것이다. 이 코드를 개선하면 다음과 같아진다.
...
생성자 내에서 이 객체의 프로퍼티를 정의하고 있다. 이러한 작업을 **초기화**라고 한다. 이를 통해서 **코드의 재사용성**이 대폭 높아졌다.
